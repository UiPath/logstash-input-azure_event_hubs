:plugin: azure_event_hubs_uipath
:type: input
:default_codec: plain
...
===== Recommended: Use the Kafka input plugin

Benefits of switching to the Kafka input plugin:

* **Avoid upcoming GPv2 storage cost increases.** Microsoft will https://learn.microsoft.com/en-us/azure/storage/common/general-purpose-version-1-account-migration-overview[retire general-purpose v1 (GPv1) storage accounts in October 2026]. Accounts will be automatically migrated to GPv2, which can bring significantly higher transactional costs for checkpoint-heavy workloads (up to ~140x more expensive per write operation). Switching to the Kafka input plugin eliminates this concern entirely. No storage account means nothing to upgrade and nothing to pay for.
* **No storage costs for checkpoints.** The Kafka protocol handles offset tracking internally within the Azure Event Hubs service at no extra cost. You no longer need to provision, maintain, or pay for an Azure Blob Storage account.
* **Better throughput.** The Kafka input plugin delivers 1.7x to 2.5x higher throughput compared to this plugin under the same conditions.

Azure Event Hubs natively supports the https://learn.microsoft.com/en-us/azure/event-hubs/azure-event-hubs-kafka-overview[Apache Kafka protocol] on Standard, Premium, and Dedicated tiers. The Basic tier does not support the Kafka interface. To use it, a subscription upgrade is needed.
The https://www.elastic.co/guide/en/logstash/current/plugins-inputs-kafka.html[Kafka input plugin] (`logstash-input-kafka`) can connect directly to Azure Event Hubs with no extra Azure-side configuration, and is the recommended way to consume events.

For the full migration guide, including multi-hub configurations, parameter mapping, proxy setup, and FAQ, see the blog post: https://www.elastic.co/observability-labs/blog/migrate-logstash-pipelines-from-azure-event-hubs-to-kafka-plugin[Migrate Logstash Pipelines from Azure Event Hubs to Kafka Input Plugin].

====== Quick migration example (single Event Hub)

Before (Azure Event Hubs input):
...
Alternatively, use an inline JAAS configuration instead of a file:
...
NOTE: AMQP and Kafka consumer groups are completely independent, even if they share the same name. When the Kafka input plugin connects for the first time, it will not resume from where this plugin left off. To avoid reprocessing data, set `auto_offset_reset => "latest"` for the initial deployment.

====== Not ready to migrate yet?

If you are not ready to migrate, you can reduce the impact of GPv2 transactional costs by increasing the <<plugins-{type}s-{plugin}-checkpoint_interval>> setting above its default of `5` seconds. This reduces write operations to the Blob Storage account and lowers costs. You can estimate the cost difference using the https://azure.microsoft.com/en-us/pricing/calculator/[Azure Pricing Calculator].
